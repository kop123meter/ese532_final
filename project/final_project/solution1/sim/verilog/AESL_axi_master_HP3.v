...
//------------------------Parameter----------------------
`define TV_IN_HP3 "../tv/cdatafile/c.hardware_encoding.autotvin_HP3.dat"
`define TV_OUT_HP3 "../tv/rtldatafile/rtl.hardware_encoding.autotvout_HP3.dat"
 parameter HP3_ADDR_BITWIDTH = 32'd 64;
...
parameter HP3_C_DATA_BITWIDTH = 32'd 16;
parameter HP3_mem_depth = 32'd 32772;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [HP3_DATA_BITWIDTH - 1 : 0] HP3_mem_0 [0: HP3_mem_depth - 1]; 
reg [HP3_DATA_BITWIDTH - 1 : 0] HP3_mem_1 [0: HP3_mem_depth - 1]; 
reg [HP3_DATA_BITWIDTH - 1 : 0] HP3_mem_2 [0: HP3_mem_depth - 1]; 
reg [HP3_DATA_BITWIDTH - 1 : 0] HP3_mem_3 [0: HP3_mem_depth - 1]; 
reg [31 : 0] clk_counter ;
reg [31 : 0] current_AW_req_transaction = 0 ;
reg [31 : 0] current_AR_req_transaction = -1 ;
reg write_processing = 0;
reg [HP3_ADDR_BITWIDTH - 1 : 0] wr_processing_addr = 0;
reg [HP3_ADDR_BITWIDTH - 1 : 0] rd_processing_addr = 0;
reg [31 : 0] wr_processing_trans_num = 0;
reg [HP3_DATA_BITWIDTH - 1 : 0] wr_processing_data = 0;
...
initial begin : initialize_HP3_mem
  integer i;
  for (i = 0; i < HP3_mem_depth; i = i + 1) begin
      HP3_mem_0[i] = 0;
      HP3_mem_1[i] = 0;
      HP3_mem_2[i] = 0;
      HP3_mem_3[i] = 0;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_HP3_AWVALID && TRAN_HP3_AWREADY && ((FIFO_AW_req_ptr_w - FIFO_AW_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
          FIFO_AW_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_HP3_ARVALID && TRAN_HP3_ARREADY && ((FIFO_AR_req_ptr_w - FIFO_AR_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
          FIFO_AR_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_HP3_WVALID && TRAN_HP3_WREADY && ((FIFO_WDATA_req_ptr_w - FIFO_WDATA_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_HP3_WVALID && TRAN_HP3_WREADY && TRAN_HP3_WLAST && ((FIFO_WDATA_size_ptr_w - FIFO_WDATA_size_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
        ...
        end else if (send_one_BRESP != 'b1 && (BRESP_counter != 32'h0 && TRAN_HP3_BREADY)) begin
            BRESP_counter <= BRESP_counter - 1;
        end
    end
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > HP3_mem_depth) begin
                $display ("/mnt/castor/seas_home/l/lize1/ese532_final/project/final_project/solution1/sim/verilog/AESL_axi_master_HP3.v: Write request address %d exceed AXI master HP3 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, HP3_mem_depth); 
                $finish;
            end
            ...
            if (FIFO_AW_req_BURST_tmp === 1) begin
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        0 : WDATA_tmp[j] = HP3_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                                        1 : WDATA_tmp[j] = HP3_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                                        2 : WDATA_tmp[j] = HP3_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                                        3 : WDATA_tmp[j] = HP3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            0 : HP3_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            1 : HP3_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            2 : HP3_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            3 : HP3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
                    ...
                    if (FIFO_WDATA_req_empty != 1) begin 
                        ...
                        for (i = 0 ; i < data_byte_size; i = i + 1) begin
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        0 : WDATA_tmp[j] = HP3_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        1 : WDATA_tmp[j] = HP3_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        2 : WDATA_tmp[j] = HP3_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        3 : WDATA_tmp[j] = HP3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        default: $display("The page_num of AXI write is not valid!");
                                    endcase
                                end
                            end
                        ...
                        wr_processing_data = WDATA_tmp;
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            0 : HP3_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            1 : HP3_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            2 : HP3_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            3 : HP3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > HP3_mem_depth) begin
                $display ("/mnt/castor/seas_home/l/lize1/ese532_final/project/final_project/solution1/sim/verilog/AESL_axi_master_HP3.v: Read request address %d exceed AXI master HP3 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, HP3_mem_depth); 
                $finish;
            end
            ...
            if (FIFO_AR_req_BURST_tmp === 1) begin
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            0 : RDATA_tmp <= HP3_mem_0[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                            1 : RDATA_tmp <= HP3_mem_1[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                            2 : RDATA_tmp <= HP3_mem_2[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                            3 : RDATA_tmp <= HP3_mem_3[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            0 : RDATA_tmp <= HP3_mem_0[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            1 : RDATA_tmp <= HP3_mem_1[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            2 : RDATA_tmp <= HP3_mem_2[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            3 : RDATA_tmp <= HP3_mem_3[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            default: $display("The page_num of AXI read is not valid!");
                        endcase
                    RVALID_tmp <= 1;
                    RRESP_tmp <= 0;
      ...
      for(i = 0; i < HP3_mem_depth; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  case(mem_page)
                      0 : HP3_mem_0[i/factor] = mem_tmp;
                      1 : HP3_mem_1[i/factor] = mem_tmp;
                      2 : HP3_mem_2[i/factor] = mem_tmp;
                      3 : HP3_mem_3[i/factor] = mem_tmp;
          ...
          if (factor == 1) begin
              mem_tmp = token_tmp;
              case(mem_page)
                  0 : HP3_mem_0[i] = mem_tmp;
                  1 : HP3_mem_1[i] = mem_tmp;
                  2 : HP3_mem_2[i] = mem_tmp;
                  3 : HP3_mem_3[i] = mem_tmp;
                  default: $display("The page_num of read file is not valid!");
              endcase
              mem_tmp [HP3_DATA_BITWIDTH - 1: 0] = 0;
          end
...
initial begin : write_file_proc 
  ...
  reg [HP3_DATA_BITWIDTH - 1 : 0] tmp_data;
  transaction_num = 0; 
  HP3_bitwidth = HP3_C_DATA_BITWIDTH; 
  count_seperate_factor_by_bitwidth (HP3_bitwidth , factor);
  while(1) begin 
      ...
      for(i = 0; i < (HP3_mem_depth - HP3_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  0 : tmp_data = HP3_mem_0[i/factor];
                  1 : tmp_data = HP3_mem_1[i/factor];
                  2 : tmp_data = HP3_mem_2[i/factor];
                  3 : tmp_data = HP3_mem_3[i/factor];
          ...
          if (factor == 1) begin
              case(mem_page)
                  0 : mem_tmp = HP3_mem_0[i];
                  1 : mem_tmp = HP3_mem_1[i];
                  2 : mem_tmp = HP3_mem_2[i];
                  3 : mem_tmp = HP3_mem_3[i];
                  default: $display("The page_num of write file is not valid!");
              endcase
              $fdisplay(fp,"0x%x",mem_tmp);
          end
      ...
      if (factor == 4) begin
          case(mem_page)
              0 : tmp_data = HP3_mem_0[HP3_mem_depth/factor];
              1 : tmp_data = HP3_mem_1[HP3_mem_depth/factor];
              2 : tmp_data = HP3_mem_2[HP3_mem_depth/factor];
              3 : tmp_data = HP3_mem_3[HP3_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
      ...
      if (factor == 2) begin
          if ((HP3_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : $fdisplay(fp,"0x%x",HP3_mem_0[HP3_mem_depth/factor][15:0]);
                  1 : $fdisplay(fp,"0x%x",HP3_mem_1[HP3_mem_depth/factor][15:0]);
                  2 : $fdisplay(fp,"0x%x",HP3_mem_2[HP3_mem_depth/factor][15:0]);
                  3 : $fdisplay(fp,"0x%x",HP3_mem_3[HP3_mem_depth/factor][15:0]);
                  default: $display("The page_num of write file is not valid!");
              endcase
          end
      end
